...
export default function CreateSickRequest(props) {
  ...
  const handleClick = (e) => {
    ...
    requestBody.compensationDate = requestDetails.compensationDate
    ...
    axios({
      method: 'POST',
      url: `${backendLink}/request/compensationLeaveRequest`,
      headers: {
        'auth-token': token,
      },
      data: requestBody,
      ...
      <Card className='createRequest-card'>
        <Card.Body>
          <Container>
            <Row xs={12}>
              <Col xs={12}>
                <p className='createRequest-header'>
                  Compensation Request Information
                </p>
              </Col>
            </Row>
            <Row>
            ...
            <Row>
              <Col xs={3}>
                <p className='createRequest-label'>Compensation Date*</p>
              </Col>
              <Col xs={9}>
                <input
                  className='createRequest-input'
                  value={requestDetails.compensationDate}
                  onChange={handleChange}
