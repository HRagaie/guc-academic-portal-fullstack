import React, { useState, useEffect } from 'react'
import axios from 'axios'
import backendLink from '../../keys_dev'
import ListView from '../ListView'
import CancelImage from '../../Images/cancel.png'
import AcceptImage from '../../Images/AcceptImage.png'
import RejectImage from '../../Images/RejectImage.png'
import Filters from '../Filters'
import { useHistory } from 'react-router-dom'
import CreateReplacementRequest from '../Schedule/ReplacementReqSchedule'

import '../../Styles/Requests.css'

export default function SlotLinkningRequests() {
  const token = localStorage.getItem('Token')
  const memberId = localStorage.getItem('memberId')
  const memberType = localStorage.getItem('type')
  const history = useHistory()
  const [sentReplacementRequests, setSentReplacementRequests] = useState([])
  const [
    receivedReplacementRequests,
    setReceivedReplacementRequests,
  ] = useState([])
  const [headers, setHeaders] = useState([])
  const [attributes, setAttributes] = useState([])
  const [actions, setActions] = useState([])
  const [stateChanged, setStateChanged] = useState(false)
  const [requestState, setRequestState] = useState('sent')
  const [error, setError] = useState('')
  const [avaialbleCourses, setAvailableCourses] = useState([])
  const [modalState, setModalState] = useState({
    show: false,
    type: '',
    handle: '',
    id: '',
  })

  const [filters, setFilters] = useState([
    {
      type: 'dropdown',
      label: 'Status',
      name: 'status',
      options: [
        {
          value: 'all',
          text: 'All Requests',
        },
        {
          value: 'accept',
          text: 'Accepted',
        },
        {
          value: 'reject',
          text: 'Rejected',
        },
        {
          value: 'pending',
          text: 'Pending',
        },
        {
          value: 'cancelled',
          text: 'Cancelled',
        },
      ],
    },
  ])

  const [filterObject, setFilterObject] = useState({
    status: '',
  })

  const handleCancel = (id, index) => {
    axios({
      method: 'POST',
      url: `${backendLink}/request/cancelReplacementRequest`,
      headers: {
        'auth-token': token,
      },
      data: {
        requestId: id,
      },
    })
      .then((response) => {
        if (response.data.code) {
          setError(response.data.message)
        } else {
          setStateChanged(!stateChanged)
          setModalState({
            show: false,
            type: '',
            handle: '',
            id: '',
          })
        }
      })
      .catch((err) => {
        console.log(err.response)
        setError(err.response.data.message)
      })
  }

  const handleAccept = (id, index) => {
    axios({
      method: 'POST',
      url: `${backendLink}/request/acceptReplacementRequest`,
      headers: {
        'auth-token': token,
      },
      data: {
        requestId: id,
      },
    })
      .then((response) => {
        console.log(response)
        if (response.data.code) {
          setError(response.data.message)
        } else {
          setStateChanged(!stateChanged)
          setError('')
          setModalState({
            show: false,
            type: '',
            handle: '',
            id: '',
          })
        }
      })
      .catch((err) => {
        console.log(err.response)
        setError(err.response.data.message)
      })
  }
  const handleReject = (id, comment) => {
    const reqBody = {}
    reqBody['requestId'] = id
    if (comment) reqBody['comment'] = comment
    // console.log(reqBody)
    axios({
      method: 'POST',
      url: `${backendLink}/request/rejectReplacementRequest`,
      headers: {
        'auth-token': token,
      },
      data: reqBody,
    })
      .then((response) => {
        console.log(response)
        if (response.data.code) {
          setError(response.data.message)
        } else {
          setStateChanged(!stateChanged)
          setError('')
          setModalState({
            show: false,
            type: '',
            handle: '',
            id: '',
          })
        }
      })
      .catch((err) => {
        console.log(err.response)
      })
  }

  useEffect(() => {
    const reqBody = {}
    if (filterObject['status'] !== '' && filterObject['status'] !== 'all')
      reqBody['status'] = filterObject['status']
    axios({
      method: 'POST',
      url: `${backendLink}/request/viewReplacementRequests`,
      headers: {
        'auth-token': token,
      },
      data: reqBody,
    })
      .then((response) => {
        console.log(response)
        setError('')
        const sent = response.data.myRequests
        const received = response.data.replacementRequests
        sent.forEach((element) => {
          element.memberName = element.member.name
          element.slotDetails = `Slot ${element.slot.slot} on ${element.slot.day} Course ${element.slot.course.name} in ${element.slot.room.name}`
          element.replacementMemberName = element.replacementMember.name
          element.replacementDate = element.dateOfReplacement.split('T')[0]
        })
        setSentReplacementRequests(sent)
        received.forEach((element) => {
          element.memberName = element.member.name
          element.slotDetails = `Slot ${element.slot.slot} on ${element.slot.day} Course ${element.slot.course.name} in ${element.slot.room.name}`
          element.replacementMemberName = element.replacementMember.name
          element.replacementDate = element.dateOfReplacement.split('T')[0]
        })
        setReceivedReplacementRequests(received)
        setHeaders([
          'Replacement Date',
          'Member',
          'Replacement',
          'Slot',
          'Status',
          'Comment',
          'Action',
        ])
        setAttributes([
          'replacementDate',
          'memberName',
          'replacementMemberName',
          'slotDetails',
          'status',
          'comment',
        ])
        if (requestState === 'sent') {
          setActions([
            {
              src: CancelImage,
              handle: handleCancel,
              type: 'cancel',
            },
          ])
        } else {
          setActions([
            {
              src: AcceptImage,
              handle: handleAccept,
              type: 'accept',
            },
            {
              src: RejectImage,
              handle: handleReject,
              type: 'reject',
            },
          ])
        }
      })
      .catch((err) => {
        console.log(err.response)
      })
  }, [stateChanged, filterObject, requestState])

  return (
    <div className='main-page'>
      <p className='request-heading'>Replacement Requests</p>
      <div className='horizontalLine'>
        <button
          className='borderTop-button'
          onClick={() => setRequestState('sent')}
          autoFocus
        >
          Sent Requests
        </button>
        <button
          className='borderTop-button'
          onClick={() => setRequestState('received')}
        >
          Received Requests
        </button>
        <button
          className='borderTop-button'
          onClick={() => setRequestState('add')}
        >
          Send Replacement Request
        </button>
      </div>

      {requestState !== 'add' ? (
        <div>
          <Filters
            filters={filters}
            setFilters={setFilters}
            filterObject={filterObject}
            setFilterObject={setFilterObject}
          />
          <ListView
            list={
              requestState === 'sent'
                ? sentReplacementRequests
                : receivedReplacementRequests
            }
            headers={headers}
            attributes={attributes}
            actions={actions}
            error={error}
            setError={setError}
            modalState={modalState}
            setModalState={setModalState}
          />
        </div>
      ) : (
        <CreateReplacementRequest />
      )}
    </div>
  )
}
