...
const getDepartment = async (req, res) => {
  try {
    ...
    if (memberType !== memberRoles.HR) {
      const memberFound = await Member.findById(memberId)
      if (memberFound.department.toString() !== departmentId) {
        return res.status(403).json({
          code: unauthorized,
          message: 'Not Authorized to see this department details',
        })
      }
    }
    const department = await departmentModel
      .findById(departmentId)
      .populate(['faculty', 'headOfDepartment'])
    return res.json({
      department,
    })
  } catch (err) {
