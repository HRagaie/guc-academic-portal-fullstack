...
const addMember = async (req, res) => {
  try {
    ...
    const createdMember = new MemberModel(member)
...
const updateMember = async (req, res) => {
  try {
    ...
    if (req.body.email) {
      ...
      if (checkEmail && checkEmail._id.toString() !== memberId) {
        return res.status(400).json({
          code: emailAlreadyExists,
          message: 'Email Already Exists',
        })
    ...
    if (req.body.office && memberFound.office.toString() !== req.body.office) {
      const officeFound = await RoomModel.findById(req.body.office)
      if (!officeFound) {
        return res.status(404).json({
          code: roomDoesnotExist,
    ...
    if (req.body.dayoff) {
      if (
        memberFound.type === memberRoles.HR &&
        req.body.dayoff !== weekDays.SATURDAY
      )
        return res.status(400).json({
          code: dayOffError,
          message: 'Dayoff Must Be Saturday for HR.',
        })
    }
    memberFound.save()
    return res.json({
...
const viewMissingDaysHours = async (req, res) => {
  try {
    ...
    for (
      ...
      promisesArray.push(
        attendanceRecordsCheck(DayRecords, new Date(DayTime - stepTime), member)
      )
      // console.log('ans', x)
    }
    const results = await Promise.all(promisesArray)
    for (let i = 0; i < results.length; i++) {
      ans.missingDays += results[i].missingDays
      ans.time += results[i].time
    }
    return ans
...
const ViewScadule = async (req, res) => {
  try {
    ...
    let result = {}
    result.Member = me
    const ReplacmentRecords = await replacementRequest
      .find({
        replacementMember: req.member.memberId,
