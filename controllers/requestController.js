...
const rejectSlotLinkingRequest = async (req, res) => {
  try {
    ...
    const reqBody = {}
    reqBody['status'] = requestType.REJECT
    if (req.body.comment) reqBody['comment'] = req.body.comment
...
const viewSlotLinkning = async (req, res) => {
  try {
    ...
    const filterObject = {}
    if (req.body.status) filterObject['status'] = req.body.status

    const records = await SlotLinkingModel.find(filterObject)
      .populate({
        path: 'slot',
        match: {
          course: courseId,
...
const viewMySlotLinking = async (req, res) => {
  try {
    ...
    const checkMemberOnCourse = await CourseAssignmentModel.findOne({
      member: memberId,
      course: courseId,
    })
    if (!checkMemberOnCourse) {
      return res.status(403).json({
        code: memberNotAssignedToCourse,
        message: 'Member Not on this course',
      })
    }

    const filterObject = {}
    if (req.body.status) filterObject['status'] = req.body.status
    filterObject['member'] = memberId
    const records = await SlotLinkingModel.find(filterObject)
      .populate({
        path: 'slot',
        match: {
...
const accidentalLeaveRequest = async (req, res) => {
  try {
    ...
    if (obj.absentDate > new Date()) {
      return res.status(400).json({
        message: 'Absent Date Must Be In The Past',
        code: pastAbsentDate,
      })
    }
...
const accidentalRejectRequest = async (req, res) => {
  try {
    ...
    if (req.body.comment) reqBody['comment'] = req.body.comment
    reqBody.status = requestType.REJECT
    await AccidentalLeaveRequest.findByIdAndUpdate(requestId, reqBody)
...
const sendAnnualLeave = async (req, res) => {
  try {
    ...
    if (req.body.replacements) {
      newReq['replacements'] = []
      req.body.replacements.forEach(async (replacement) => {
        const replacementFound = await replacementRequest.findById(replacement)
        if (!replacementFound) {
          return res.json({
            code: requestDoesNotExist,
            message: 'Request Does Not Exist!',
        ...
        newReq.replacements.push(replacement)
      })
    }
...
const cancelCompensationLeaveRequest = async (req, res) => {
  try {
    ...
    if (tokenId !== compensationRequest.member.toString()) {
      return res.status(403).json({
        code: notYourRequest,
        message: 'You can only cancel your requests',
      })
    }
    ...
    CompensationModel.findByIdAndUpdate(
      ...
      function (err) {
        if (err) {
          ...
          return res.json({
            message: 'Compensation Request Cancelled',
          })
        }
      }
    )
...
const viewAnnualRequests = async (req, res) => {
  try {
    ...
    if (checkDep.type === memberRoles.HOD && req.body.type === 'department') {
      ...
      const records = await annualLeaveRequest
        .find(filterObj)
        .populate('member')
      return res.json({
        data: records,
...
const viewAccidentalRequests = async (req, res) => {
  try {
    ...
    if (req.body.status) filterObj['status'] = req.body.status
    if (checkDep.type === memberRoles.HOD && req.body.type === 'department') {
      const records = await AccidentalLeaveRequest.find(filterObj).populate({
        path: 'member',
        match: {
          department: checkDep.department,
        },
...
const viewCompensationRequests = async (req, res) => {
  try {
    ...
    if (req.body.status) filterObj['status'] = req.body.status
    if (checkDep.type === memberRoles.HOD && req.body.type === 'department') {
      const records = await CompensationModel.find(filterObj).populate({
        path: 'member',
        match: {
          department: checkDep.department,
        },
      ...
      filterObj.member = tokenId
      const records = await CompensationModel.find(filterObj).populate('member')
      return res.json({
        data: records,
      })
    }
...
const viewChangeDayOffRequests = async (req, res) => {
  try {
    ...
    if (req.body.status) filterObj['status'] = req.body.status
    if (checkDep.type === memberRoles.HOD && req.body.type === 'department') {
      const records = await Request.find(filterObj).populate({
        path: 'member',
        match: {
          department: checkDep.department,
        },
      ...
      filterObj.member = tokenId
      const records = await Request.find(filterObj).populate('member')
      return res.json({
        data: records,
      })
    }
...
const viewSickRequests = async (req, res) => {
  try {
    ...
    if (req.body.status) filterObj['status'] = req.body.status
    if (checkDep.type === memberRoles.HOD && req.body.type === 'department') {
      const records = await sickRequest.find(filterObj).populate({
        path: 'member',
        match: {
          department: checkDep.department,
        },
      ...
      filterObj.member = tokenId
      const records = await sickRequest.find(filterObj).populate('member')
      return res.json({
        data: records,
      })
    }
...
const viewMaternityRequests = async (req, res) => {
  try {
    ...
    if (req.body.status) filterObj['status'] = req.body.status
    if (checkDep.type === memberRoles.HOD && req.body.type === 'department') {
      const records = await maternityRequest.find(filterObj).populate({
        path: 'member',
        match: {
          department: checkDep.department,
        },
      ...
      filterObj.member = tokenId
      const records = await maternityRequest.find(filterObj).populate('member')
      return res.json({
        data: records,
      })
    }
...
const acceptReplacementRequests = async (req, res) => {
  try {
    const tokenId = req.member.memberId
    const requestId = req.body.requestId
    const replacementFound = await Replacment.findById(requestId)
    if (!replacementFound)
      return res.json({
        code: requestDoesNotExist,
        message: 'Request Does Not Exist',
    ...
    if (replacementFound.status !== requestType.PENDING)
      return res.json({
        code: requestNotPending,
        message: 'Request Not Pending',
      })

    if (tokenId !== replacementFound.replacementMember.toString())
      return res.json({
        code: notYourRequest,
        message: 'You Must Be Replacement Member To Accept',
      })

    await Replacment.findByIdAndUpdate(requestId, {
      $set: { status: requestType.ACCEPT },
    })
    return res.json({
...
const rejectReplacementRequests = async (req, res) => {
  try {
    const tokenId = req.member.memberId
    const requestId = req.body.requestId
    const replacementFound = await Replacment.findById(requestId)
    ...
    const reqBody = {}
    reqBody['status'] = requestType.REJECT
    req.body.comment ? (reqBody['comment'] = req.body.comment) : null
    await Replacment.findByIdAndUpdate(requestId, {
      $set: reqBody,
    })
    return res.json({
...
const cancelReplacementRequests = async (req, res) => {
  try {
    const tokenId = req.member.memberId
    const requestId = req.body.requestId
    const replacementFound = await Replacment.findById(requestId)
    ...
    if (tokenId !== replacementFound.member.toString())
      return res.json({
        code: notYourRequest,
...
const viewReplacementRequests = async (req, res) => {
  try {
    ...
    //   })
    const filterObject = {}
    filterObject['member'] = tokenId
    if (req.body.status) filterObject['status'] = req.body.status
    const myRequests = await Replacment.find(filterObject)
      .populate(['member', 'replacementMember'])
      .populate({
        path: 'slot',
        populate: ['room', 'course'],
      })
    const filterObject2 = {}
    filterObject2['replacementMember'] = tokenId
    if (req.body.status) filterObject2['status'] = req.body.status
    const replacementRequests = await Replacment.find(filterObject2)
      .populate(['member', 'replacementMember'])
      .populate({
        path: 'slot',
        populate: ['room', 'course'],
