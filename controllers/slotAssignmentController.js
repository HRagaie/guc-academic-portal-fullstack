...
const assignMemberToSlot = async (req, res) => {
  try {
    ...
    const checkMemberBusy = await SlotAssignmentModel.findOne({
      member: req.member.memberId,
      slot: checkAssignedSlot.slot,
      day: checkAssignedSlot.day,
    })
    if (checkMemberBusy) {
      return res.status(400).json({
        code: memberBusy,
        message: 'Member Cannot Have Two Slots At The Same Time',
      })
    }
