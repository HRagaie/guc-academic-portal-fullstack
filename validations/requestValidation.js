...
const validateSendAnnualLeaveRequest = (req, res, next) => {
  const annualLeaveSchema = Joi.object({
    ...
    to: Joi.date().required(),
    replacements: Joi.array().items(Joi.string().length(24)),
    reason: Joi.string(),
  })
...
const validateCancelCompensationLeaveRequest = (req, res, next) => {
  const cancelAnnualLeaveSchema = Joi.object({
    requestId: Joi.string().length(24).required(),
  })

  const checkSchema = cancelAnnualLeaveSchema.validate(req.body)
  if (checkSchema.error) {
    return res.status(400).json({
      code: validationError,
      message: checkSchema.error.details[0].message,
...
const validateAcceptRejectCancelReplacementRequest = (req, res, next) => {
  const accpetRejectSchema = Joi.object({
    requestId: Joi.string().length(24).required(),
    comment: Joi.string(),
  })
  const checkSchema = accpetRejectSchema.validate(req.body)
  if (checkSchema.error) {
    return res.status(400).json({
      code: validationError,
      message: checkSchema.error.details[0],
